...
public class UserController {
    ...
    public ResponseEntity<?> generateToken(@RequestBody final JwtRequest jwtRequest) throws Exception {
        ...
        final Optional<TweetUser> user = userRepoImpl.findByLoginId(userDetails.getUsername());
        return ResponseEntity.ok(new JwtResponse(token, user.get().getFirstName(), userDetails.getUsername() ,"OK"));
    }
    ...
    public ResponseEntity<?> getAllUsers() {
        return new ResponseEntity<>(userRepoImpl.getAllUser(), HttpStatus.OK);
    }
}
